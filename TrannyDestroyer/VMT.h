...
namespace toolkit
{
	...
	public:
		...
		VMTBaseManager(void* inst, unsigned offset = 0, unsigned vfuncs = 0)
		{
		...
		VMTBaseManager & HookMethod(void * newfunc, e_offset index);
	...
	inline VMTBaseManager& VMTBaseManager::HookMethod(void* newfunc, e_offset index)
	{
		return HookMethod(newfunc, (int)index);
	}
