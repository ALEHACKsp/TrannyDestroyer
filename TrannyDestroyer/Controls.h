...
public:
	...
	bool IsVisible() { return !visible || *visible; }
	...
	inline const vector<BaseControl*>& GetChildren() { return children; }
...
public:
	...
	Groupbox(const char* Name, const vector<BaseControl*>& Children, int W, int H = NULL)
	{
		...
		name = Name, children = Children, w = W, h = H, bAutoHeight = H == 0;
		flags = scale_width;
	}
};
...
public:
	...
	KeyBind(const char* Name, byte VirtualKey, bool Enabled = false, int W = 100, int X = 0, int Y = 0)
	{
		type = e_control::keybind, flags = scale_width;
		name = Name, key = VirtualKey, bEnabled = Enabled, x = X, y = Y;
		h = 14 + 14, w = W;
	}
};